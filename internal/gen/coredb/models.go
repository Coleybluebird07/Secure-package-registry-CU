// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.31.1

package coredb

import (
	"database/sql/driver"
	"fmt"

	"github.com/jackc/pgx/v5/pgtype"
)

type CollectionTaskStatus string

const (
	CollectionTaskStatusPending   CollectionTaskStatus = "pending"
	CollectionTaskStatusRunning   CollectionTaskStatus = "running"
	CollectionTaskStatusSucceeded CollectionTaskStatus = "succeeded"
	CollectionTaskStatusFailed    CollectionTaskStatus = "failed"
	CollectionTaskStatusCancelled CollectionTaskStatus = "cancelled"
)

func (e *CollectionTaskStatus) Scan(src interface{}) error {
	switch s := src.(type) {
	case []byte:
		*e = CollectionTaskStatus(s)
	case string:
		*e = CollectionTaskStatus(s)
	default:
		return fmt.Errorf("unsupported scan type for CollectionTaskStatus: %T", src)
	}
	return nil
}

type NullCollectionTaskStatus struct {
	CollectionTaskStatus CollectionTaskStatus
	Valid                bool // Valid is true if CollectionTaskStatus is not NULL
}

// Scan implements the Scanner interface.
func (ns *NullCollectionTaskStatus) Scan(value interface{}) error {
	if value == nil {
		ns.CollectionTaskStatus, ns.Valid = "", false
		return nil
	}
	ns.Valid = true
	return ns.CollectionTaskStatus.Scan(value)
}

// Value implements the driver Valuer interface.
func (ns NullCollectionTaskStatus) Value() (driver.Value, error) {
	if !ns.Valid {
		return nil, nil
	}
	return string(ns.CollectionTaskStatus), nil
}

type Ecosystem string

const (
	EcosystemNpm   Ecosystem = "npm"
	EcosystemGo    Ecosystem = "go"
	EcosystemCargo Ecosystem = "cargo"
	EcosystemPypi  Ecosystem = "pypi"
)

func (e *Ecosystem) Scan(src interface{}) error {
	switch s := src.(type) {
	case []byte:
		*e = Ecosystem(s)
	case string:
		*e = Ecosystem(s)
	default:
		return fmt.Errorf("unsupported scan type for Ecosystem: %T", src)
	}
	return nil
}

type NullEcosystem struct {
	Ecosystem Ecosystem
	Valid     bool // Valid is true if Ecosystem is not NULL
}

// Scan implements the Scanner interface.
func (ns *NullEcosystem) Scan(value interface{}) error {
	if value == nil {
		ns.Ecosystem, ns.Valid = "", false
		return nil
	}
	ns.Valid = true
	return ns.Ecosystem.Scan(value)
}

// Value implements the driver Valuer interface.
func (ns NullEcosystem) Value() (driver.Value, error) {
	if !ns.Valid {
		return nil, nil
	}
	return string(ns.Ecosystem), nil
}

type PkgVtype string

const (
	PkgVtypeInteger PkgVtype = "integer"
	PkgVtypeBoolean PkgVtype = "boolean"
	PkgVtypeFloat   PkgVtype = "float"
)

func (e *PkgVtype) Scan(src interface{}) error {
	switch s := src.(type) {
	case []byte:
		*e = PkgVtype(s)
	case string:
		*e = PkgVtype(s)
	default:
		return fmt.Errorf("unsupported scan type for PkgVtype: %T", src)
	}
	return nil
}

type NullPkgVtype struct {
	PkgVtype PkgVtype
	Valid    bool // Valid is true if PkgVtype is not NULL
}

// Scan implements the Scanner interface.
func (ns *NullPkgVtype) Scan(value interface{}) error {
	if value == nil {
		ns.PkgVtype, ns.Valid = "", false
		return nil
	}
	ns.Valid = true
	return ns.PkgVtype.Scan(value)
}

// Value implements the driver Valuer interface.
func (ns NullPkgVtype) Value() (driver.Value, error) {
	if !ns.Valid {
		return nil, nil
	}
	return string(ns.PkgVtype), nil
}

type RebuildTaskStatus string

const (
	RebuildTaskStatusPending     RebuildTaskStatus = "pending"
	RebuildTaskStatusRunning     RebuildTaskStatus = "running"
	RebuildTaskStatusSucceeded   RebuildTaskStatus = "succeeded"
	RebuildTaskStatusFailed      RebuildTaskStatus = "failed"
	RebuildTaskStatusCancelled   RebuildTaskStatus = "cancelled"
	RebuildTaskStatusUnavailable RebuildTaskStatus = "unavailable"
)

func (e *RebuildTaskStatus) Scan(src interface{}) error {
	switch s := src.(type) {
	case []byte:
		*e = RebuildTaskStatus(s)
	case string:
		*e = RebuildTaskStatus(s)
	default:
		return fmt.Errorf("unsupported scan type for RebuildTaskStatus: %T", src)
	}
	return nil
}

type NullRebuildTaskStatus struct {
	RebuildTaskStatus RebuildTaskStatus
	Valid             bool // Valid is true if RebuildTaskStatus is not NULL
}

// Scan implements the Scanner interface.
func (ns *NullRebuildTaskStatus) Scan(value interface{}) error {
	if value == nil {
		ns.RebuildTaskStatus, ns.Valid = "", false
		return nil
	}
	ns.Valid = true
	return ns.RebuildTaskStatus.Scan(value)
}

// Value implements the driver Valuer interface.
func (ns NullRebuildTaskStatus) Value() (driver.Value, error) {
	if !ns.Valid {
		return nil, nil
	}
	return string(ns.RebuildTaskStatus), nil
}

type Account struct {
	ID                    string
	AccountId             string
	ProviderId            string
	UserId                string
	AccessToken           pgtype.Text
	RefreshToken          pgtype.Text
	IdToken               pgtype.Text
	AccessTokenExpiresAt  pgtype.Timestamptz
	RefreshTokenExpiresAt pgtype.Timestamptz
	Scope                 pgtype.Text
	Password              pgtype.Text
	CreatedAt             pgtype.Timestamptz
	UpdatedAt             pgtype.Timestamptz
}

type Apikey struct {
	ID                  string
	Name                pgtype.Text
	Start               pgtype.Text
	Prefix              pgtype.Text
	Key                 string
	ConfigId            string
	ReferenceId         string
	RefillInterval      pgtype.Int4
	RefillAmount        pgtype.Int4
	LastRefillAt        pgtype.Timestamptz
	Enabled             bool
	RateLimitEnabled    bool
	RateLimitTimeWindow pgtype.Int4
	RateLimitMax        pgtype.Int4
	RequestCount        int32
	Remaining           pgtype.Int4
	LastRequest         pgtype.Timestamptz
	ExpiresAt           pgtype.Timestamptz
	CreatedAt           pgtype.Timestamptz
	UpdatedAt           pgtype.Timestamptz
	Permissions         pgtype.Text
	Metadata            pgtype.Text
}

type CollectionTask struct {
	ID               int32
	PackageVersionID int32
	Source           string
	Status           CollectionTaskStatus
	WorkflowRunID    pgtype.Int8
	ArtifactBucket   pgtype.Text
	ArtifactKey      pgtype.Text
	StartedAt        pgtype.Timestamptz
	HeartbeatAt      pgtype.Timestamptz
	CompletedAt      pgtype.Timestamptz
	FailureReason    pgtype.Text
	CreatedAt        pgtype.Timestamptz
	UpdatedAt        pgtype.Timestamptz
}

type Invitation struct {
	ID             string
	OrganizationId string
	Email          string
	Role           pgtype.Text
	Status         string
	ExpiresAt      pgtype.Timestamptz
	CreatedAt      pgtype.Timestamptz
	InviterId      string
}

type Member struct {
	ID             string
	OrganizationId string
	UserId         string
	Role           string
	CreatedAt      pgtype.Timestamptz
}

type Organization struct {
	ID        string
	Name      string
	Slug      string
	Logo      pgtype.Text
	CreatedAt pgtype.Timestamptz
	Metadata  pgtype.Text
}

type OrganizationPackage struct {
	ID             int32
	OrganizationID string
	PackageID      int32
}

type Package struct {
	ID                   int32
	Identifier           string
	Ecosystem            Ecosystem
	LatestVersion        pgtype.Text
	MaintainerTrustLevel pgtype.Int4
	CreatedAt            pgtype.Timestamptz
	UpdatedAt            pgtype.Timestamptz
}

type PackageTagType struct {
	ID          int32
	Label       string
	Description pgtype.Text
	ValueType   PkgVtype
	CreatedAt   pgtype.Timestamptz
}

type PackageVersion struct {
	ID               int32
	PackageID        int32
	Version          string
	SourceUrl        pgtype.Text
	SourceTag        pgtype.Text
	SourceCommitHash pgtype.Text
	MaintainerNotes  pgtype.Text
	CreatedAt        pgtype.Timestamptz
	UpdatedAt        pgtype.Timestamptz
}

type PackageVersionTag struct {
	PackageVersion int32
	TagType        int32
	Value          []byte
	CreatedAt      pgtype.Timestamptz
	UpdatedAt      pgtype.Timestamptz
}

type RebuildTask struct {
	ID                     int32
	PackageVersionID       int32
	Source                 string
	Status                 RebuildTaskStatus
	Matched                pgtype.Bool
	OfficialArtifactBucket pgtype.Text
	OfficialArtifactKey    pgtype.Text
	RebuiltArtifactBucket  pgtype.Text
	RebuiltArtifactKey     pgtype.Text
	DiffoscopeBucket       pgtype.Text
	DiffoscopeKey          pgtype.Text
	LogsBucket             pgtype.Text
	LogsKey                pgtype.Text
	MetadataBucket         pgtype.Text
	MetadataKey            pgtype.Text
	StartedAt              pgtype.Timestamptz
	HeartbeatAt            pgtype.Timestamptz
	CompletedAt            pgtype.Timestamptz
	FailureReason          pgtype.Text
	CreatedAt              pgtype.Timestamptz
	UpdatedAt              pgtype.Timestamptz
}

type Session struct {
	ID                   string
	ExpiresAt            pgtype.Timestamptz
	Token                string
	CreatedAt            pgtype.Timestamptz
	UpdatedAt            pgtype.Timestamptz
	IpAddress            pgtype.Text
	UserAgent            pgtype.Text
	UserId               string
	ActiveOrganizationId pgtype.Text
}

type User struct {
	ID            string
	Name          string
	EmailVerified bool
	Image         pgtype.Text
	CreatedAt     pgtype.Timestamptz
	UpdatedAt     pgtype.Timestamptz
}

type Verification struct {
	ID         string
	Identifier string
	Value      string
	ExpiresAt  pgtype.Timestamptz
	CreatedAt  pgtype.Timestamptz
	UpdatedAt  pgtype.Timestamptz
}
